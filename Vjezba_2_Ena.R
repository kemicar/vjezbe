...
write.table(x=df2,
            ...
            sep=";") #;,\t(tab) pipe(I)
getwd()
tablica<-read.table(file="data_frame.txt",
                    sep=";",
                    header=TRUE,
                    stringsAsFactor=T)
view(tablica)
df3<-read.table(file="data_frame.txt",
                sep=";",
                header=TRUE,
                stringsAsFactor=T)
view(df3)
df4<-read.table(file="direktorij",
                sep="\t",
                header=F)
view(df4)
...
library("readxl")
c<-read_excel("excel.xlsl")
view(c)
d<-read_excel("excel.xlsl",sheet = "sheet_2")
view(d)
